...
struct hash_table_entry {
	...
	pthread_mutex_t mutex; //mutex per list
...
{
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	int lockres = pthread_mutex_lock(&hash_table_entry->mutex);
	if (lockres != 0)
	  {
	    exit(lockres);
	  }
	...
	if (list_entry != NULL) {
		...
		int unlockres = pthread_mutex_unlock(&hash_table_entry->mutex);
		if (unlockres != 0)
		  {
		    exit(unlockres);
		  }
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		struct hash_table_entry *entry = &hash_table->entries[i];
		int destroyres = pthread_mutex_destroy(&entry->mutex);
		if (destroyres != 0)
		  {
		    exit(destroyres);
		  }
